<template>
    <main class="main">
        ...
        <div class="container-fluid px-4">
                    ...
                    <tr v-for="carrera in carreras" :key="carrera.id">
                        ...
                        <td id="icons-pos">
                            <div class="button-container">
                                <button type="button" @click="abrirModal('editar', carrera)" data-toggle="modal"
                                    data-target="#carreersModal" class="btn btn-warning btn-sm" style="width: 100%;">
                                    <i class="icon-pencil"></i>
                                    <span class="btn-label">Editar</span>
                                </button>
                            </div>
                            <div class="button-container">
                                <button v-if="carrera.estado == 1" type="button"
                                    @click="abrirModal('eliminar', carrera)" data-toggle="modal"
                                    data-target="#statusModal" class="btn btn-danger btn-sm"
                                    style="margin: 2px 0; width: 100%;">
                                    <i class="icon-lock"></i>
                                    <span class="btn-label">Desactivar</span>
                                </button>
                                <button v-else type="button" @click="abrirModal('activar', carrera)" data-toggle="modal"
                                    data-target="#statusModal" class="btn btn-success btn-sm"
                                    style="margin: 2px 0; width: 100%;">
            ...
            <button class="btn btn-success mb-3" @click="abrirModal('insertar', null)" data-toggle="modal"
                data-target="#carreersModal">Nueva carrera</button>
            <!--Inicio del modal-->
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form>
                                <select class="form-control custom-select" v-model="modal_facultad" required>
                                    ...
                                    <option v-for="facultad in facultades" :value="facultad.idFacultad"
                                        :key="facultad.nombre">{{ facultad.nombre }}</option>
                                </select>
                                <input class="form-control" type="text" v-model="nombre_carrera" placeholder="Carrera"
                                    required>
                            </form>
                        </div>
                        <div class="modal-footer" style="border-top: none;">
                            ...
                            <button type="button" class="btn btn-success"
                                ...
                                @click="actualizarInsertarCarrera()">Guardar</button>
                        </div>
                    </div>
                    <!-- /.modal-content -->
                </div>
...
export default {
    ...
    methods: {
        ...
        async actualizarInsertarCarrera() {
            const me = this;

            try {

                if (!this.idCarrera) {
                    const response = await axios.post(`${API_HOST}/carreras/insertar`, {
                    ...
                    const response = axios.put(`${API_HOST}/carreras/actualizar`, {
                        'idCarrera': this.idCarrera,
                        'idFacultad': this.modal_facultad,
                        'nombre': this.nombre_carrera,
                        'estado': 1
        ...
        async inactivarCarrera(idCarrera) {
            let estado = 0;
            const me = this;

            try {                
                await axios.put(`${API_HOST}/carreras/inactivar`, {
                    'idCarrera': idCarrera,
                    'estado': estado
                })
                // .then(function (response) {
        ...
        async activarCarrera(idCarrera) {
            let estado = 1;
            const me = this;
